...
def main():
    ...
    r = sub.add_parser("run", help="تشغيل التحليل وتوليد الإشارات")
    r.add_argument("--symbols", default=None, help="BTC,ETH,SOL")
    r.add_argument("--json", action="store_true")
    r.set_defaults(fn=cmd_run)
    s = sub.add_parser("signals", help="عرض آخر الإشارات")
    s.add_argument("--limit", type=int, default=20)
    s.set_defaults(fn=cmd_signals)
    b = sub.add_parser("backtest", help="اختبار استراتيجية سريع")
    b.add_argument("--symbols", default="BTC,ETH,SOL")
    b.set_defaults(fn=cmd_backtest)
    d = sub.add_parser("dashboard", help="تشغيل لوحة المتابعة")
